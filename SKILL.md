...
- **Reuse Mode**: when the user asks whether something can be reused, asks to make an implementation reusable, asks for reusable component/module documentation, asks how to copy it into another project, or asks how to package it for reuse, read `references/rules/reuse-mode.md` before designing or editing. Reuse Mode first assesses whether the target has real reusable properties; if it is reusable, produce the reuse documentation directly, and if it is not reusable or incomplete, explain the gaps, propose concrete reusable refactoring, wait for user confirmation, then refactor and document. Reuse is not asking users to copy scattered code snippets: organize reuse around self-contained files, folders, tool modules, components, or packages that can be copied/installed, imported, configured, and called. Reuse Mode requires executable reuse paths, not aspirational advice, and the `## 复用方式` section must be detailed enough for a brand-new project to introduce the asset through copy/install, import, config, data, event/callback, style/layout, and verification steps.
